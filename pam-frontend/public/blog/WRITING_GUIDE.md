...
1. **연차 있는 개발자 어투** — 경험을 나누는 톤. "알아보겠습니다"식 설명자 역할 금지.
2. **자기 객관화** — 삽질·실수를 담담하게 기술한다. 방어적으로 합리화하지 않는다.
3. **`...` 겸손 표현** — 확신이 낮거나 여운을 남길 때 말줄임표를 쓴다.
   - 예: "그게 전부인 줄 알았다..."
4. **`~다` 서술체 고수** — 시리즈 전체가 이 어투로 통일되어 있으므로 변경하지 않는다.

---

### 6-2. 도입부 패턴

**도입부는 아래 두 패턴 중 하나로 시작한다.** 추상적 설명이나 인사말로 시작하지 않는다.

**A. 일기형 현장 도입** — 사건이 일어난 순간의 장면을 현재형으로 서술해 독자를 끌어당긴다.

```
로그아웃 버튼을 누르면 토스트가 뜬다. 새로고침하면 다시 로그인 상태다.
1차에서 멈췄으면 좋았겠지만, 결국 5차까지 갔다.
```

**B. 자기 의문형 도입** — 문제 설정을 의문문으로 던지고, 스스로 의문이 들었다고 서술한다.

```
"모든 결과 이미지를 저장해야 하나?" 하는 생각이 들었다.
처음 설계는 단순했다. 그런데 그 단순함이 틀렸다.
```
...
- **기본: 2~3문장.** 4문장을 넘으면 분리를 검토한다.
- `<p>` 하나 = 하나의 생각. 여러 생각을 한 단락에 묶지 않는다.
...
- 한 `<p>` 안에 `<strong>` 최대 1회.
- 문장 전체를 감싸지 않는다. 핵심 단어나 구(1~3개) 수준만.
- 강조가 과도하면 모두 강조되지 않은 것과 같다.
...
| 금지 표현 | 이유 |
|---|---|
| `~하는 방법을 알아보겠습니다` | 설명 블로그 어투 |
| `결론적으로 말씀드리면` | 격식체 |
| `오늘은 ~에 대해 공유드리려 합니다` | 인사말형 인트로 |
| `사실 이 부분이 핵심입니다!` | 감탄 과잉 |
| `어렵지 않으니 따라해보세요` | 교습형 어투 |
| `lesson-box`에 추상 교훈 | "보안은 중요하다" 류 — 반드시 행동 강령으로 |
...
- `lesson-box`는 "다음에 이렇게 할 것이다" 형태의 **행동 강령**으로 닫는다.
  - ❌ "보안은 중요하다"
  - ✅ "다음 프로젝트에선 인증 흐름을 배포 환경에서 먼저 검증한다"
- 글의 마지막 문장은 짧게 끊는다. 긴 결론 문장으로 늘어뜨리지 않는다.
